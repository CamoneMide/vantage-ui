...
- [x] **Create mock history fixtures**: Define typed history data with realistic entries.
...
- [x] **Create Zustand history slice**: State management for extraction history.
...
- [x] **Build `HistoryEmpty` component**: Empty state for when no extractions exist.
...
- [x] **Build `HistoryItem` component**: Single row card in the history list.
...
- [x] **Install `date-fns` for timestamp formatting**: Required for `formatDistanceToNow`.
...
- [x] **Build delete confirmation popover**: Prevents accidental deletion.
...
- [x] **Build re-open extraction action**: Re-populates Sandpack with a saved extraction.
...
- [x] **Assemble `HistoryTab` and wire into side panel**: Replace placeholder from Phase 5.
