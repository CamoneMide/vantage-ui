...
- [ ] **Scaffold Plasmo app**: Initialize the Plasmo extension inside `apps/extension/`.
...
- [ ] **Register Side Panel in manifest**: Configure the `manifest.json` override for the Chrome Side Panel.
...
- [ ] **Register the keyboard shortcut**: Add `Ctrl+Shift+X` for toggling the inspector.
...
- [ ] **Create popup shell (`src/popup.tsx`)**: Minimal popup with placeholder content.
...
- [ ] **Create side panel shell (`src/sidepanel.tsx`)**: Minimal side panel with placeholder content.
...
- [ ] **Create content script shell (`src/contents/inspector.tsx`)**: Minimal content script that logs activation.
...
- [ ] **Wire keyboard shortcut to content script**: Connect the `Ctrl+Shift+X` command to the content script.
...
- [ ] **Add `dev` script to workspace**: Add a convenience script in root `package.json`.
