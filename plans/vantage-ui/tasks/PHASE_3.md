...
- [ ] **Initialize `packages/ui` package**: Set up the package manifest and TypeScript config.
...
- [ ] **Configure Tailwind CSS with VantageUI tokens**: Create the custom Tailwind config.
...
- [ ] **Set up global CSS base styles**: Create `packages/ui/src/globals.css`.
...
- [ ] **Load fonts**: Configure Outfit, DM Sans, and JetBrains Mono.
...
- [ ] **Initialize Shadcn/ui**: Run the Shadcn CLI and configure components.
...
- [ ] **Apply VantageUI theme overrides to Shadcn**: Ensure Shadcn uses VantageUI tokens.
...
- [ ] **Create `packages/ui/src/index.ts`**: Central export barrel.
...
- [ ] **Create `CreditBadge` primitive**: A reusable credit balance badge used across popup and side panel.
...
- [ ] **Create `StepperProgress` primitive**: Used in the extraction flow (Phase 8).
