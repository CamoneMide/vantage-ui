...
- [x] **Initialize pnpm monorepo**: Create a `pnpm-workspace.yaml` at the repo root defining three workspaces.
...
- [x] **Configure root TypeScript**: Set up a `tsconfig.base.json` at the root with strict mode enabled.
...
- [x] **Configure ESLint (Airbnb + TypeScript)**: Set up a root `.eslintrc.cjs` with Airbnb React config and TypeScript plugin.
...
- [x] **Configure Prettier**: Add a `.prettierrc` at root.
...
- [x] **Configure Vitest at root**: Install and configure Vitest for the monorepo.
...
- [x] **Verify clean dev environment**: Confirm the monorepo structure is sound.
