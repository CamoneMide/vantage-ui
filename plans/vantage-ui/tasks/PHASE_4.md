...
- [ ] **Set up popup entry point styling**: Apply design system to the popup root.
...
- [ ] **Build `PopupHeader` component**: Renders the VantageUI logo/wordmark + credit balance badge.
...
- [ ] **Build unauthenticated popup state**: Auth gate for first-time / logged-out users.
...
- [ ] **Build authenticated popup state**: Main popup dashboard for logged-in users.
...
- [ ] **Build low-credit warning ribbon**: Renders when credit balance falls below 5.
...
- [ ] **Build dev auth toggle**: Hidden dev mode flag for toggling auth state during testing.
...
- [ ] **Write unit test for `CreditBadge` color logic**: Vitest unit test.
